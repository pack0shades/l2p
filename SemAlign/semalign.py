...
class SemAlign_SBERT(nn.Module):
    def __init__(self, v_size, s_size, h_size=2048, drop=0.0):
        super(SemAlign_SBERT, self).__init__()
...
class SemAlign_BERT(nn.Module):
    def __init__(self, v_size, s_size, h_size=2048, drop=0.0):
        super(SemAlign_BERT, self).__init__()
        self.model = nn.Sequential(
            nn.Linear(v_size + s_size, h_size),
            nn.LeakyReLU(0.2),
            nn.Linear(h_size, 1024),    # Now both inputs are of size v_size
            nn.LeakyReLU(0.2)
        )
        self.drop = nn.Dropout(drop)
        self.fc = nn.Linear(1024, v_size)
